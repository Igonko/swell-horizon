...
const TableProducts: FC<TableProductProps> = ({
  ...
  const [defaultState, setDefaultState] = useState<IBodyTable | undefined>(undefined);
    ...
    if (
      ...
      const selectProduct = size?.map((item) => {
        if (item.name === name) {
          return {
            ...item,
            productOption: {...item.productOption, quantity: +productCount},
          };
        }
        return item;
      });
      ...
      if (updateProduct && Object.keys(updateProduct)?.length) {
        const totalItems: number =
          Object.entries(updateProduct)?.reduce((acc, item) => {
            return (
              acc +
              item[1].size.reduce((count, el) => {
        ...
        const totalPrice: number =
          Object.entries(updateProduct)?.reduce((acc, item) => {
            return (
              ...
              item[1].size.reduce((count, el) => {
                return (
                  count + Number(el.price) * Number(el.productOption.quantity)
                );
              }, 0)
            );
          }, 0) || 0;
      ...
      updateProduct &&
      Object.keys(updateProduct)?.length &&
      setBodyTable(updateProduct);
    }
  };
        ...
        item[1].size.forEach((sizeItem) => {
          if (sizeItem.productOption.quantity > 0) {
            getProductForSend.push({
              options:
                sizeItem.productOption.selectedProductOptions &&
                Object.keys(sizeItem.productOption.selectedProductOptions)?.map(
                  (item) => ({
                    id: item,
                    valueId:
                      sizeItem.productOption.selectedProductOptions[item],
                  }),
                ),
  ...
  const createSortOptions = (arr: ProductOption[], searchName: string) => {
    if (arr?.length && searchName) {
      return arr
        ?.filter(
          (item) => (item.name as string).toLowerCase() === `${searchName}`,
        )[0]
        ?.values?.reduce(
          (acc, item) => ({
            ...acc,
            [item.id]:
              searchName === 'color' ? {...item, size: []} : item.name,
          }),
          {},
        );
    }
  ...
  useEffect(() => {
    if (productOptions?.length && purchaseOptions?.standard?.price) {
      const header = productOptions
        ...
        ?.values?.map((h) => ({
          ...h,
          price: h.price + purchaseOptions?.standard?.price,
        }));
  ...
  useEffect(() => {
    if (productVariants?.length && productOptions?.length) {
      const sizes: Record<string, string> | undefined = createSortOptions(
        productOptions,
        'size',
      );
      const sortProductColor: IBodyTable | undefined = createSortOptions(
        productOptions,
        'color',
      );
      const options: Record<string, Record<string, string>> = {};
      const sortProduct: string[] | undefined =
        sizes && Object.keys(sizes).map((item) => item);
      ...
      if (
        ...
        for (let i = 0; i < productVariants?.length; i++) {
          const searchColor = productVariants[i]?.optionValueIds?.find(
            (item) => options.color[item as string],
          );
          const searchSize = productVariants[i]?.optionValueIds?.find(
            (item) => options.size[item as string],
          );
      ...
      const sortSizeProduct =
        sortProductColor &&
        Object.entries(sortProductColor)?.reduce((acc, item) => {
      ...
      if (sortSizeProduct &&
        Object.keys(sortSizeProduct)?.length) {
        setBodyTable(sortSizeProduct);
        setDefaultState(sortSizeProduct)
      }
  ...
  return (
    <div>
          ...
          <div className="inline-block min-w-full py-2 sm:px-6">
            <div className="overflow-x-auto">
              <table
                ...
                <tr>
                  ...
                  {headerTable?.map((item) => (
                    <th
                      ...
                      <div> {item.name}</div>
                      <div>
                        <span className="mt-2 text-sm text-body">$</span>
                        {item.price}
                      </div>
                    </th>
                  ))}
                ...
                {bodyTable &&
                  Object.keys(bodyTable)?.length &&
                  Object.entries(bodyTable)?.map((itemBody, i) => (
                    <tr key={itemBody[0]}>
                      <td
                        ...
                        <Image
                          src={itemBody[1]?.image?.file?.url as string}
                          width="40px"
                          height="40px"
                        />
                      </td>
                      ...
                      {itemBody[1]?.size?.map((item) => (
                        <Cell
                          ...
                          key={item.id}
                          val={item.productOption.quantity as number}
      ...
      {showTotalPriceCount ? (
        <div className="mt-5 flex flex-wrap justify-between gap-2 py-5">
          <div>
            <div>Total Items : {totalProductPrice.totalItems}</div>
            <div>
              Total Price : {totalProductPrice.totalPrice} {currency}{' '}
            </div>
          </div>
          <div className="max-[580px]:text-sm xs:mt-5 flex w-[480px] flex-wrap gap-2">
            <Button
            ...
            <Button
              ...
              {addLabel} - <Price price={totalProductPrice.totalPrice}/>
              {'Total'}
            </Button>
          </div>
        </div>
