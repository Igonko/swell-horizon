...
const useProductSelection = ({
  ...
  const addToCartProducts = async (products: CartItemInput[] | CartItemInput, config: AddToCartConfig = {showCartAfter: false}) => {
    try {
      if (Array.isArray(products) && products?.length) {
        return await Promise.all(products.map((item) => _addToCart(item, config)))
      }
      await addToCart();
      // TODO: Standardize error data type to handle errors consistently.
    } catch (error) {
